...
class ModernChart(QChartView):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(f"background-color: {COLORS['card_bg'].name()}; border-radius: 10px;")
...
class StatsCounter(QFrame):
    ...
    def __init__(self, title, color=COLORS['accent'], parent=None):
        ...
        # Заголовок
        title_label = QLabel(title)
        title_label.setStyleSheet(f"color: {COLORS['text_secondary'].name()}; font-size: 14px; font-weight: bold;")
        title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        # Значение
        self.value_label = QLabel("0")
        self.value_label.setStyleSheet(f"color: {color.name()}; font-size: 32px; font-weight: bold;")
        self.value_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
...
class ScrapTFApp(QMainWindow):
    def __init__(self):
        ...
        self.system_stats_worker.stats_updated.connect(self.update_system_stats)
    ...
    def create_ui(self):
        ...
        bottom_splitter.setStyleSheet(f"QSplitter::handle {{ background-color: {COLORS['card_bg'].lighter(130).name()}; }}")
        ...
        raffle_stats_header.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {COLORS['text'].name()};")
        ...
        console_header.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {COLORS['text'].name()};")
    ...
    def update_system_stats(self, stats):
        ...
        self.memory_card.details_label.setText(f"{stats['memory_used']:.1f} / {stats['memory_total']:.1f} ГБ")
        ...
        self.network_card.details_label.setText(f"Получено: {stats['net_recv']:.1f} МБ")
        self.network_card.progress_bar.setValue(min(int(stats['net_recv'] / 10 * 100), 100))  # 10 МБ = 100%
